...
\begin{itemize}
	\item Verificación: transmitir tramas de datos pesadas (miles de lecturas simuladas) a través del puerto de comunicación USB desde el microcontrolador hacia la PC, verificando la recepción de un archivo con los datos.
	\item Validación: El investigador del departamento de Biología Molecular conecta el equipo a su PC y comprueba que puede extraer sin problemas las planillas de resultados, verificando que los formatos (columnas, tiempos, valores) le resultan útiles para su análisis de laboratorio.
\end{itemize}
...
\begin{itemize}
	\item Verificación: programar un temporizador (un \textit{timer} interno) en el microcontrolador con timers internos para cronometrar la ejecución exacta desde la interrupción del botón de inicio hasta la finalización del ensayo.
